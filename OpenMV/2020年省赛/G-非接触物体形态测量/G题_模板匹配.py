...
tilt_pid = PID(p=0.01, i=0, imax=90)#在线调试使用这个PID0.05
...
def Automatic_tracking(thresholds,start3):
    global m,n,t,blob,thresholds_1,once
    ...
    if delta<=30:
        ...
        if blobs:
            ...
            Multi_color_tracking(thresholds,start3)
...
#要求四----------------------------------------------------------------------------------------------
#多颜色组合追踪-----------------------------------------------
def three_dimensional(img,threshold_Ball,start4):
    global t, m, n, D, E, F, once
    #计时
    delta = int(utime.ticks_diff(utime.ticks_ms(), start4)/1000)
    print('时间',delta)

    blobs = img.find_blobs(threshold_Ball, pixels_threshold=200, area_threshold=200)
    if delta<=30:
        if blobs:
            ...
            for t in templates:
                ...
                if r:
                    ...
                    pan_error = r[0]-img.width()/2
                    tilt_error = r[1]-img.height()/2
                    img.draw_cross(r[0], r[1]) # cx, cy 最大b色域的色块中心画十字
                    ...
                    D += 1
                    print(t)
                    if t == ("/0.pgm" or "/1.pgm" or "/2.pgm" or "/3.pgm" or "/4.pgm"):
                        if D>100:
                            once=1
                            send_data_packet(0x06,R,length)

                    if t == ("/00.pgm" or "/11.pgm" or "/22.pgm" or "/33.pgm" or "/44.pgm"):
                        if D>100:
                            once=1
                            send_data_packet(0x05,R,length)

                    if t == ("/000.pgm" or "/111.pgm" or "/222.pgm" or "/333.pgm" or "/444.pgm"):
                        if D>100:
                            once=1
        ...
        if once == 0:
            ...
            onlyone1=xingzhuang[int(random.randint(0,2))]
            onlyone=int(random.randint(4,5))
            #测距
            length=TOF()
            #测边长
...
threshold_Ball = [(0, 52, -128, 127, 7, 127),#棕红色
...
#附加部分-------------------------------------------------
#激光阈值
laser_threshold = (28, 100, -128, 127, -128, 127)
...
while(True):
    ...
    if not pin3.value():
        ...
        if a==0 and i==0 :
            i+=1
            a=1
            start1 = utime.ticks_ms() # get value of millisecond counter  获取毫秒计数器的值
        elif a==1 and i==0:
            i+=1
            a=2
            start2 = utime.ticks_ms() # get value of millisecond counter  获取毫秒计数器的值

        elif a==2 and i==0 :
            i+=1
            a=3
            start3 = utime.ticks_ms() # get value of millisecond counter  获取毫秒计数器的值
        elif a==3 and i==0:
            i+=1
            a=4
            start4 = utime.ticks_ms() # get value of millisecond counter  获取毫秒计数器的值
        elif a==4 and i==0:
            i+=1
            a=5
            m=0
