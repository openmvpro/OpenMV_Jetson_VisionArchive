...
Frame_Cnt = 0
fCnt_tmp = [0,0]
def send_data_packet(FormType, Loaction0, Location1):
    global Frame_Cnt
    global fCnt_tmp
    Frame_Head = [170,170]
    Frame_End = [85,85]
...
def Color_combination():
    thresholds = [(0, 100, -128, -23, -128, 127), #            色块位: 1
                    (0, 100, 20, 127, -128, 127)] #               2      若有第四组色块位为:8
...
def Chasing_ball(last_blob):
    ...
    if now_blob:
        ...
        print(blobs)
        blob=find_max(blobs)
        x_error = blob[5]-img.width()/2
        length=Proportional_ranging(1125,blob)
