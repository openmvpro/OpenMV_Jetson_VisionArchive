...
{      			    
	...
	chr=chr-'0';//得到偏移后的值		 
	for(t=0;t<csize;t++)							//显示点阵库的中的该字符，先通过chr=chr-' '确定字符在点阵库中的行数，接着显示字符所在行的全部数据
	{   
		if(size==16)
			temp=asc2_1616[chr][t];	//调用1608字体
		for(t1=0;t1<8;t1++)							//显示显示字符所在行的全部数据
		{
			if(temp&0x80)
				OLED_DrawPoint(x,y,mode);	//显示一个字节的数据从高位开始开始判断，如果为1则在x,y位置点亮，为0不点亮
			else 
				OLED_DrawPoint(x,y,!mode);
			temp<<=1;   								//左移一位，为扫描下一个点数据
			y++;     										//格点下移
			if((y-y0)==size)            //控制y轴最大扫描数
			{
				y=y0;
				x++;
				break;
...
void OLED_ShowChinese2424(u16 x,u16 y,u8 index,u8 size)
{
	...
	if(size == 12) 
		dzk=(u8*)Chinese_1212[index];        //得到汉字编号对应的点阵库 
	else if(size == 16)	
		dzk=(u8*)Chinese_1616[index];   //得到汉字编号对应的点阵库 
	else if(size == 24) 
		dzk=(u8*)Chinese_2424[index];   //得到汉字编号对应的点阵库 
//	dzk=(u8*)Chinese_2424[index];        //得到汉字编号对应的点阵库 
	for(t=0;t<csize;t++)
	{                                                                                                      
		temp=dzk[t];                                //得到点阵数据                          
...
void OLED_Showfloat(u8 x,u8 y,float num,u8 size)     //显示float
{
	char temp[10];
  sprintf(temp,"%-5.3f",num);
  OLED_ShowString(x,y,(u8 *)temp,size);
}
...
{ 	 		 
  ...
  __HAL_RCC_GPIOE_CLK_ENABLE();   //使能GPIOE时钟
...
#else					//使用4线SPI 串口模式
		//GPIO初始化设置      
    GPIO_Initure.Pin=GPIO_PIN_4;         //PA15
    GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;//推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;        //上拉
    ...
    //PC6,7
    GPIO_Initure.Pin=GPIO_PIN_7|GPIO_PIN_12|GPIO_PIN_13;	
	  HAL_GPIO_Init(GPIOD,&GPIO_Initure);//初始化	
